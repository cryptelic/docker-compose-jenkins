...
def create(name, template):
    ...
    result = _get_cf_connection().create_stack('%s-%s-%s' % (name,
                                                             datetime.now().strftime('%Y%m%d'),
                                                             datetime.now().strftime('%H%M%S%z')),
                                               template_body=_get_template(
                                                   template).to_json(),
...
def _get_cf_connection():
    conn = \
        cloudformation.connect_to_region(os.environ['AWS_DEFAULT_REGION'
                                                    ])
    try:
        conn.region
        return conn
    except AttributeError:
        print("Unable to obtain a Cloud Formation connection. Review the README to ensure you've provided the correct environment to connect to AWS.")
        exit()
